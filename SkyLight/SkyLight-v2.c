...
static Bool skyIsCorrect(int x, int y, int dir)
{
	y *= CELLW;
	uint8_t sky = skyLight[x+y], max, val, i, flags;
	if (sky == WALL) return True;

	for (i = max = 0, flags = dirs[dir]; i < 4; i ++, flags >>= 1)
	{
		if ((flags & 1) == 0) continue;
		val = skyLight[x+xoff[i]+y+yoff[i]*CELLW];
		if (i >= 2 && val == MAXSKY) val = MAXSKY+1;
		if (val != WALL && max < val) max = val;
	}
	if (max > 0) max --;
	return sky == max;
}
		...
		if (sky != WALL)
		{
			...
			if (sky != max)
			{
				...
				*cell = max;
				...
				{
					if ((flags & 1) == 0) continue;
					if (! skyIsCorrect(xsky+xoff[i], ysky+yoff[i], i))
						addTrack(xsky + xoff[i] - xstart, ysky + yoff[i] - ystart, i);
				}
			}
		}
	}
...
int my_main(int nb, char * argv[])
{
	...
	SIT_CreateWidgets(dialog,
		...
		"<button name=full title='Full' left=WIDGET,reset,0.5em buttonType=", SITV_ToggleButton, "curValue=", &fullStep, ">"
